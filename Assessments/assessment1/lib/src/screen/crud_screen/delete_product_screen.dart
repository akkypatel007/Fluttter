...
class _DeleteScreenState extends State<DeleteScreen> {
  ...
  InsertScreen insertScreen = InsertScreen();
              ...
              SingleChildScrollView(
                  child: SizedBox(
                      ...
                      child: FutureBuilder<List<Notes>>(
                        future: databaseHelper.getAllDeletedNotes(),
          ...
          child: Card(
            ...
            child: Container(
              ...
              child: ListTile(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          noteList[position].price as String,
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 16,
                          ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          noteList[position].quantity as String,
                          style: const TextStyle(
                            color: Colors.white,
                            fontSize: 16,
                          ),
                ...
                onTap: () {
                  debugPrint("ListTile Tapped");
                  navigateToDetail('edit', noteList[position]);
                },
              ),
            ),
          ),
  ...
  void navigateToDetail(String s, Notes notes) async {
    bool result =
        await Navigator.push(context, MaterialPageRoute(builder: (context) {
      return InsertScreen(
        appBarTitle: s, /*note: notes*/
      );
    }));
  ...
  void _deletes(BuildContext context, Notes noteList) async {
    DataBaseHelper dataBaseHelper = DataBaseHelper();
    print("hey--------------${noteList.id}");
    print("hey--------------${noteList.product}");
    print("hey--------------${noteList.price}");
    print("hey--------------${noteList.category}");
    print("hey--------------${noteList.quantity}");
    print("hey--------------${noteList.date}");
    int results;
    notes.product = noteList.product;
    notes.category = noteList.category;
    notes.price = noteList.price;
    notes.quantity = noteList.quantity;
    notes.date = noteList.date;

    if (deletedNotes.id != null) {
      // Case 1: Update operation
      // Case 2: Insert Operation
      print("hey--------------${notes.id}");
      print("hey--------------${notes.product}");
      print("hey--------------${notes.price}");
      print("hey--------------${notes.category}");
      print("hey--------------${notes.quantity}");
      print("hey--------------${notes.date}");
      await databaseHelper.addDeletedNote(notes);
      results = await databaseHelper.updateNote(notes);
    }
    int result = await dataBaseHelper.deleteNoteById(noteList.id);
    if (noteList.deleted == 1) {
      noteList.id;
    }
    if (result != 0) {
      // ignore: use_build_context_synchronously
...
//   } else {
//     // Case 2: Insert Operation
//     result = await databaseHelper.addDeletedNote(deletedNotes);
//   }
//   if (result != 0) {
//     // Success
